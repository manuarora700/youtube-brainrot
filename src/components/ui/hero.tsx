...
const PATHS = [
    ...
    { d: "M 164 0 L 98.814 0 L 0 83.557 L 0 205", transform: "translate(400 110)" },
    {
...
const AnimationContainer = () => {
    return (
        <div className="pointer-events-none absolute inset-x-0 -bottom-20 z-40 mx-auto w-full max-w-[734px]">
            <div className="size-52 absolute -bottom-20 inset-x-0 mx-auto bg-white/20 p-2 rounded-md">
                <div className="size-full bg-white"></div></div>
            <Tag className="absolute inset-x-0 mx-auto">Company page Visits</Tag>
            <Tag className="absolute top-22 mx-auto">LinkedIn Followers</Tag>
            <Tag className="absolute top-22 right-0">Post Engagers</Tag>
            <Tag className="absolute top-50 -right-20">Post Commenters</Tag>
            <Tag className="absolute top-50 -left-20">Post Visitors</Tag>
            <svg
                role="presentation"
                viewBox="0 0 734 405"
...
const Tag = ({ children, className }: { children: React.ReactNode, className?: string }) => {
    return <div className={cn("px-4 py-2 bg-linear-to-tr from-white/50 to-transparent border border-white text-neutral-700 backdrop-blur-sm  font-light uppercase font-mono w-60 text-center", className)}>{children}</div>
}
